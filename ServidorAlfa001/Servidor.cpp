...
<a type=\"submit\" href=\"/restaura/\"> - Restaurar - </a>\
</form>\
...
void setup_Server() {
  ...
  server.on("/restaura/", handle_Restaura);
...
bool ServerRestauraHistorial = false;
void handle_Restaura() {
  ServerRestauraHistorial = true;
  server.send(200, "text/html", SendHTML(ArchivoDatos.TemperaturaAmbiente, ArchivoDatos.HumedadAmbiente, ArchivoDatos.PresionAmbiente, ArchivoDatos.Altitud, FechaHTML, ArchivoDatos.PrediccionPresion));
 
}
bool ServerRestaura() {
  if (ServerRestauraHistorial == false) {
    return false;
  }
  else {
    ServerRestauraHistorial = false;
    return true;
  }
